...
- [13. 교착 상태](#13-교착-상태)
  - [1. 교착 상태란](#1-교착-상태란)
    - [1. 교착 상태란? - 식사하는 철학자 문제](#1-교착-상태란---식사하는-철학자-문제)
...
### 1. 교착 상태란? - 식사하는 철학자 문제

- 식사하는 철학자 문제
  - 교착 상태가 왜 발생하는지 나아가서는 교착 상태를 어떤 방식으로 해결할 수 있는지 까지도 힌트를 얻을 수 있는 좋은 문제 상황.
    - 동그란 식탁에 5명의 철학자가 앉아있다.
    - 철학자들 사이사이에 포크가 있다. 음식을 먹으려면 2개의 포크를 이용해야 한다.
    ...
    6. 다시 1번부터 반복한다.
  - 모든 철학자들이 동시에 이러한 순서대로 식사를 하게 되면 결국에 그 어떤 철학자도 식사를 할 수가 없게 된다. 결국 생각만 하게 되는 지경에 이름.
- 일어나지 않을 사건들을 계쏙 기다리면서 진행이 멈추어버리는 현상이 교착 상태이다.
- 교착 상태를 해결하기 위해서는?
  1.  첫째, 교착 상태가 발생했을 때의 상황을 정확히 표현해보기
  2.  둘째, 교착 상태가 일어나는 근본적인 이유 이해하기
...
### 3. 교착 상태가 발생할 조건
    ...
    네 가지 조건을 모두 만족하면 교착 상태가 발생할 수 있다.
...
- 교착 상태 발생 조건(상호 배제, 점유와 대기, 비선점, 원형 대기) 중 하나를 없애버리기
  ...
  - 원형 대기 조건을 없앤다?
    ...
    - 어떤 자원에 어떤 번호를 붙이느냐에 따라 활용룰이 달라진다.
...
- e.g. 안전상태
  ...
  - 안전 순서열이 존재 : P2 -> P1 -> P3
    - 프로세스 P1, P2, P3이 모두 최대로 자원을 요구하는 최악의 상황 가정 (각각 5개, 2개, 7개의 자원을 요구)
    1. P2는 이미 자원 두 개를 가지고 있으므로 남은 자원에서 두 개를 배분
       - 할당한 자원 : 11
       - 남은 자원 : 1
    2. 요구한 자원을 할당 받은 P2는 정상적으로 작업을 끝낸 뒤 갖고 있던 자원을 반환
       - 할당한 자원 : 11 - 4 = 7
       - 남은 자원 : 1 + 4 = 5
    3. P1에게 남은 자원 5개 할당 -> P1 또한 정상적으로 작업 완료
       - 할당한 자원 : 7 + 5 = 12
       - 남은 자원 : 5 - 5 = 0
    4. P1이 작업을 마치고 자원을 반환하면 P3에게 자원을 할당
       - 할당한 자원 : 12 - 10 = 2
       - 남은 자원 : 0 + 10 = 10
  - 즉, P2 -> P1 -> P3이라는 안전 순서열대로 할당하면 모든 프로세스 실행 가능

- 다른 e.g. 불안전 상태
  - P1 : 5개, P2 : 2개, P3 : 3개를 할당 받은 경우
    - 할당한 자원 : 10
    - 남은 자원 : 2
    - 요구하는 자원 : (P1:5개), (P2:2개), (P3:6개)
    1. 남은 자원이 2개라 P2에게 할당해줄 수밖에 없다.
    2. P2의 실행 종료 후 반환 돼서 남을 자원은 4개
    3. P1에게도, P3에게도 할당할 수 없음.
  - 교착 상태 발생
  - 안전 순서열이 존재하지 않음.
...
- 현재 사용되지 않는 프로세스들을 보조기억장치의 일부 영역(스왑 영역)으로 쫓아내고(스왑 아웃) 그렇게 생긴 빈 공간에 새 프로세스 적재(스왑 인)
- 프로세스들이 요구하는 메모리 공간 크기 > 실제 메모리 크기
  - 이 경우에도 프로세스들을 동시에 실행할 수 있는 이점을 가지고 있다.
...
- 외부 단편화 (external fragmentation)이라는 문제가 발생하기 때문이다.
  ...
  - e.g. 사용자 영역이 200MB일 때
    - 크기가 50MB인 프로세스 A
    - 30MB인 프로세스 B
    - 100MB인 프로세스 C
    - 20MB인 프로세스 D를 차례대로 적재해야 한다면?
    - ABCD가 순서대로 적재. -> B와 D를 사용하지 않아 스왑 아웃 한다면
    - B가 있던 자리 30MB, D가 있던 자리 20MB가 비게 된다.
    - 빈 공간이 총 50MB이지만 50MB의 크기를 필요로 하는 프로세스가 적재될 공간은 없다.
- 외부 단편화 해결
  1. 메모리 압축(compaction)
     - 여기저기 흩어져 있는 빈 공간들을 하나로 모으는 방식
     - 프로세스를 적당히 재배치시켜 흩어져 있는 작은 빈 공간들을 하나의 큰 빈 공간으로 만드는 방법
     - 재배치하는 과정에서 많은 오버헤드 발생
  2. 가상 메모리 기법, 페이징 (현대 운영체제에서 선택하는 가장 대중적인 방식)
...
### 2. 페이징이란?
    ...
    각기 다른 크기의 프로세스가 메모리에 연속적으로 할당 되었기 때문.
...
- 문제
  - 프로세스를 이루는 페이지가 어느 프레임에 적재되어 있는지 CPU가 일일이 알기 어렵다. (불연속적으로 적재되어 있기 때문)
...
- 내부 단편화
  - 페이징을 통해서 외부 단편화를 해결할 수 있지만 내부 단편화를 야기할 수 있다.
  - e.g. 페이지 크기가 10KB, 프로세스 크기 108KB?
    - 2KB: 내부 단편화 (남는 2KB는 낭비)
    - 하나의 페이지 크기보다 작은 크기로 발생

### 4. PTBR

- 페이지 테이블 베이스 레지스터(PTBR)
  - 프로세스마다 페이지 테이블이 있고, 각 페이지 테이블이 적재된 주소는 CPU 내의 페이지 테이블 베이스 레지스터(PTBR)가 가리킨다.
- 문제
...
- 특정 주소에 접근하고자 할 때 필요한 정보
  - 어떤 페이지/프레임에 접근하고자 하는지
  - 접근하려는 주소가 그 페이지 혹은 프레임으로부터 얼마나 떨어져 있는지
- 페이징 시스템에서의 논리 주소
  - 페이지 번호(page number)와 변위(offset)로 이루어져 있다.
- <페이지 번호, 변위>로 이루어진 논리 주소는 페이지 테이블을 통해 <프레임 번호, 변위>로 이루어진 물리 주소로 변환된다.
  - 페이지의 크기와 프레임의 크기가 같기 때문에 논리주소의 변위와 물리주소의 변위는 동일하다.
...
- 페이지 테이블의 각각의 행 : 페이지 테이블 엔트리(PTE)
  ...
  - 이외에도 담기는 정보들이 존재.
    ...
    - 현재 페이지가 스왑 영역에 있는지 메모리에 적재되어 있는지 아닌지 등을 나타낸다.
    - 유효 비트가 1이라면 메모리에 적재되어 있고, 0이라면 메모리에 적재되어 있지 않다는 의미이다.
    - e.g. 유효 비트가 0인 페이지에 접근하려고 하면?
    ...
    - 페이지의 보호 기능을 위해 존재하는 비트
      - 읽기 전용 페이지, 일기/쓰기 가능한 페이지 등
    3. 참조 비트
       - CPU가 이 페이지에 접근한 적이 있는지 여부
         - 어떤 페이지가 메모리로 적재된 후네 CPU가 한 번이라도 읽거나 쓴 페이지는 참조 비트가 1
         - 적재된 이후로 한 번도 CPU가 방문한 적 없으면 해당 페이지의 참조 비트는 0이다.
    4. 수정 비트 (=dirty bit)
       - CPU가 이 페이지에 데이터를 쓴 적이 있는지 여부
         - 비트가 1 : 한 번이라도 변경된 적 o
         - 비트가 0 : 한 번이라도 변경된 적 x
       - 존재 이유
         - 이 페이지가 메모리에서부터 없어질 때 보조기억장치의 쓰기 작업을 해야되는지 할 필요가 없는지 판단하기 위해 존재.
           - 메모리에 어떤 페이지 A라고 하는 페이지가 있고 CPU가 이 페이지를 읽기만 했다, 새로운 정보를 쓴 적이 없다라고 한다면 a는 메모리에 있는 내용과 보조기억장치에 있는 내용과 똑같다.
           - 하지만 CPU가 이 페이지의 내용을 변경했다면 이 페이지는 스왑 아웃 될 때 내용을 보조기억장치에도 반영을 해야한다.
...
### 2. 페이지 교체 알고리즘
    ...
    요구 페이징 기법으로 페이지들을 적재하다보면
    언젠가 메모리가 가득 차게 된다.
    당장 실행에 필요한 페이지를 적재하려면
    적재된 페이지를 보조기억장치로 내보내야한다.
    이때 내보낼 페이지를 결정하는 방법(알고리즘)이
    페이지 교체 알고리즘이다.

- 무엇이 좋은 페이지 교체 알고리즘인가?
  - 페이지 폴트가 적은 알고리즘
    - 페이지 폴트가 발생하면 보조기억장치에 접근해야 해서 성능 저하
- 페이지 폴트 횟수는 어떻게 알 수 있는가?
  - 페이지 참조열(page reference string)
    - CPU가 참조하는 페이지들 중 연속된 페이지를 생략한 페이지열
      - e.g. 2223555337 -> 23537

1. FIFO 페이지 교체 알고리즘

   - 가장 단순한 방식
   - 메모리에 가장 먼저 올라온 페이지부터 내쫓는 방식
   - 프로그램 실행 초기에 잠깐 실행될 페이지는 스왑 아웃 되어도 괜찮지만 프로그램 실행 내내 사용될 페이지를 먼저 적재되었다고 내쫓아서는 안된다.
...
- 페이지 폴트가 자주 발생하는 이유
  ...
  - 프로세스가 사용할 수 있는 프레임 자체가 적어서 <- 근본적인 이유
- 스래싱
  - 프로세스가 실행되는 시간보다 페이징에 더 많은 시간을 소요하여 성능(CPU 이용률)이 저하되는 문제
  - 동시 실행되는 프로세스의 수를 늘린다고 CPU 이용률이 높아지는 것이 아니다.
  - 멀티프로그래밍의 정도에 따라 CPU 이용률이 늘어나지만 무한정으로 늘어나는 것이 아닌 일정 수준을 넘어가면 그 순간 부터 CPU 이용률이 현저히 떨어지는데, 이 지점을 스래싱이 발생한 지점이라고 한다.
    - 멀티프로그래밍의 정도 : 메모리에 동시에 실행되는 프로세스의 수
  - 스래싱 발생 이유
    - 각 프로세스가 필요로 하는 최소한의 프레임 수가 보장되지 않았기 때문
    - 그렇기 때문에 운영체제는 각 프로세스가 필요로 하는 최소한의 프레임 수를 파악하고 프로세스들에게 적절한 프레임을 할당해주어야 한다.
- 프레임 할당
  - 균등 할당, 비례할당 : 정적 할당 방식
  ...
  - 비례 할당(proportional allocation)
    ...
    - 부작용
      ...
      - 프로세스 크기와 필요한 프레임의 양이 반대인 경우가 존재.
      - 결국 프로세스가 필요로 하는 프레임 수는 실행해봐야 아는 경우가 많다.
      - 그렇기 때문에 프로세스를 실행하는 과정에서 프레임을 결정하는 방식이 필요하게 되었다. 작업 집합 모델을 활용하는 방식과 페이지 폴트 빈도를 사용하는 방식.
  - 작업 집합 모델, 페이지 폴트 빈도 : 동적 할당 방식
  - 작업 집합 모델
    - 프로세스가 실행하는 과정에서 배분할 프레임 결정
    - 스래싱이 발생하는 이유는 빈번한 페이지 교체 때문
      - 그렇다고 CPU가 특정 시간 동안 주로 참조한 페이지 개수만큼만 프레임을 할당하면 된다.
...
2. 파일을 이루는 정보
   ...
   - 소유자 : 파일을 소유한 사용자를 나태낸다.
   - 위치 : 파일의 보조기억장치상의 현재 위치를 나타낸다.

3. 파일 연산을 위한 시스템 호출
   - 어떤 응용 프로그램도 임의로 파일에 접근하고 조작할 수 없으며 파일을 다루기 위해서는 운영체제한테 시스템 호출을 통해서 부탁해야한다.
   1. 파일 생성
   2. 파일 삭제
   3. 파일 열기
   4. 파일 닫기
...
4. 디렉터리 엔트리
   ...
   - 파일 내부에는 파일과 관련된 정보들이 있다면, 디렉터리의 내부에는 해당 디렉터리에 담겨 있는 대상과 관련된 정보들이 담겨 있다.
     - 이 정보는 보통 테이블(표) 형태로 구성
   - 디렉터리 엔트리
     - 각 엔트리(행)에 담기는 정보
       - 디렉터리에 포함된 대상의 이름 (파일 이름)
       - 그 대상이 보조기억장치 내에 저장된 위치(를 유추할 수 있는 정보)
       - 파일 시스템마다 차이가 있을 수 있다.
         - e.g. 디렉터리 엔트리에 파일 속성을 명시하는 경우도 있다.
...
- 이름 그대로 보조기억장치 내 연속적인 블록에 파일 할당
  - 파일A가 세 개의 블록에 걸쳐서 저장될 만한 크기면 세 개의 블록에 연속적으로 저장, 이어서 파일 B가 두 개의 블록에 걸쳐서 저장될 만한 크기면 두 개의 블록에 연속적으로 저장.
- 파일이 저장되어 있는 위치를 식별 하기 위해서 필요한 정보
  - 연속된 파일에 접근하기 위해서는 첫 번째 블록 주소와 블록 단위의 길이만 알면 된다.
  - 디렉터리 엔트리 : 파일 이름 & 블록 단위 길이 명시
    - <파일이름, 첫 번째 블록 주소, 길이>
- 부작용
...
1. 연결 할당
   ...
   - 디렉터리 엔트리 : 파일 이름 & 첫 번째 블록 주소 & 블록 단위의 길이
     - 블록의 일부에 다음 블록 주소가 저장되어 있기 때문에 첫 블록 주소만 알면 된다.
   - 단점
...
- FAT 파일 시스템
  ...
  - 연결 할당의 단점을 보완
    ...
    - FAT가 메모리에 캐시될 경우 느린 임의 속도 개선 가능
  - 디렉터리 엔트리
